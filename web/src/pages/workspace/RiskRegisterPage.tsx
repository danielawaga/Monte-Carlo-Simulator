...
import { SavedRegisters } from '../../components/workspace/SavedRegisters';
import { simulationService } from '../../services/simulationService';
import { useSimulation } from '../../state/SimulationContext';
import type { DistributionName, RegisterValidation, RiskDraft, RiskRegisterDraft, SavedRegister } from '../../types';

type RegisterTab='project'|'items'|'correlations'|'validation'|'saved';
...
export function RiskRegisterPage(){
  ...
  const {register,projectSource,canResetToImported,setRegister,startNewProject,importProject,resetToImported,setSavedRegisterId}=useSimulation();
  ...
  const openSaved=(saved:SavedRegister)=>{
    importProject(saved.register);
    // Rattacher le brouillon à sa source, pour que les exécutions y renvoient.
    setSavedRegisterId(saved.id);
    setValidation(null);setError('');
    setMessage(`« ${saved.name} » a été ouvert.`);
    setTab('project');
  };
  ...
  return <>
    ...
    <div className="register-workflow" role="tablist" aria-label="Construction du registre">
      {([['project','1','Projet'],['items','2','Postes'],['correlations','3','Corrélations'],['validation','4','Validation'],['saved','5','Enregistrés']] as const).map(([value,index,label])=><button key={value} role="tab" aria-selected={tab===value} className={tab===value?'active':''} onClick={()=>setTab(value)}><i>{index}</i><span>{label}</span>{value==='items'?<small>{activeItems.length} actif{activeItems.length>1?'s':''}</small>:null}</button>)}
    </div>
    ...
    {tab==='saved'?<SavedRegisters register={register} canPublish={canValidate} onOpen={openSaved} onSaved={setSavedRegisterId}/>:null}
